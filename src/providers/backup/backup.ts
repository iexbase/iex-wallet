...
export class BackupProvider {
    ...
    public walletDownload(password, opts, walletId: string): Promise<any> {
        return new Promise((resolve, reject) => {
            const wallet = this.walletProvider.getWallet(walletId);
            const ew = this.walletExport(password, opts, walletId);
            if (!ew) { return reject('Could not create backup'); }

            const walletName = wallet.name || wallet.id;
    ...
    public walletExport(password: string, opts, walletId: string): any {
        ...
        const wallet = this.walletProvider.getWalletAndPrivateKey(walletId);
